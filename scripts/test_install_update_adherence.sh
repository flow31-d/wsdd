...
rm -rf "$tmpdir/.codex/skills/wsd-relatorio"
rm -f "$tmpdir/.claude/commands/wsd-relatorio.md"
printf "custom loop skill\n" > "$tmpdir/.agents/skills/wsd-loop/SKILL.md"

node -e "const fs=require('fs'); const p=process.argv[1]; const c=require(p); c.version='0.4.2'; fs.writeFileSync(p, JSON.stringify(c, null, 2)+'\n')" "$tmpdir/+wsd/config.json"

"$tmpdir/+wsd/bin/wsd" update --help >"$help_out"
grep -q '^Usage: ./+wsd/bin/wsd update' "$help_out"
node -e "const c=require(process.argv[1]); if(c.version!=='0.4.2') process.exit(1)" "$tmpdir/+wsd/config.json"

"$tmpdir/+wsd/bin/wsd" update >"$update_out"
grep -q 'WSD updated: 0.4.2 ->' "$update_out"
grep -q 'Ensured: .agents/skills, .codex/skills, .claude/commands, +wsd/hooks (new files only)' "$update_out"

test -f "$tmpdir/.agents/skills/wsd-relatorio/SKILL.md"
test -f "$tmpdir/.codex/skills/wsd-relatorio/SKILL.md"
