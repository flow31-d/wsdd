...
| Arquivo vendorizado pelo WSD (`.wsd/bin/wsd`, `.wsd/hooks/*`, `.wsd/schemas/*`, `.wsd/party-mode/*`) | **Aqui (`flow31-d/wsdd`)** | Patch local some no próximo `./.wsd/bin/wsd update` |
| Skill ou comando de agente (`.codex/skills/*`, `.claude/commands/*`) | **Aqui (`flow31-d/wsdd`)** | Mesma razão: é vendorizado |
| Template (`templates/repo/*.template`, `templates/specs/*.template`) | **Aqui (`flow31-d/wsdd`)** | Para outros projetos não herdarem o mesmo bug |
| Lógica do installer (`bin/wsd-method.js`, `install.sh`) | **Aqui (`flow31-d/wsdd`)** | Está fora do projeto-alvo |
| Doc metodológica (`docs/00`–`docs/14`, `README`, `ROADMAP`, `CHANGELOG`) | **Aqui (`flow31-d/wsdd`)** | Fonte da verdade |
...
O objetivo é manter `flow31-d/wsdd` como fonte única e estável, e que cada projeto cliente saiba que recebe correções via `wsd update`, não via patches locais perpétuos.
